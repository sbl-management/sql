...
FROM customer AS c 
	INNER JOIN customer_purchases AS cs 
		ON c.customer_id = cs.customer_id 
GROUP BY c.customer_id, c.customer_last_name, c.customer_first_name
	HAVING SUM(cs.quantity * cs.cost_to_customer_per_qty) > 2000
...
*/
CREATE TABLE temp.homework_new_vendor AS
SELECT *
FROM vendor;

INSERT INTO temp.homework_new_vendor (vendor_id, vendor_name, vendor_type, vendor_owner_last_name, vendor_owner_first_name)
...
SELECT customer_id, SUM(quantity * cost_to_customer_per_qty) AS customer_spent_April_2024
FROM customer_purchases
